...
6. **Normalized Item** model (fields introduced across schema v2–v37; current [`SCHEMA_VERSION`] is **37**) + family graph
...
Schema version: **37** (`SCHEMA_VERSION`) — prior features through optional encryption (v35 / 0057) plus **multi-user concurrent review** tables (`matter_users`, locks, batches, QC samples, `items.review_version`, `matters.multi_user_enabled`) for track **0058**, plus optional `tenant_id` / OIDC link columns (v37 / **0059**). SQLite is **metadata-only** (no FTS5 primary); Tantivy segments live under `index/` via `matter-search`.
