...
//!
//! let pst = PstFile::open("archive.pst")?;
//! println!("PST: {} ({} bytes)", pst.display_name()?, pst.file_size());
//!
//! for folder in pst.folders()? {
//!     let folder = folder?;
//!     println!("Folder: {} ({} messages)", folder.name()?, folder.message_count()?);
//!
//!     for message in folder.messages()? {
//!         let msg = message?;
//!         println!("  {} — {}", msg.subject()?, msg.sender_email()?);
//!     }
//! }
//! ```
...
impl PstFile {
    ...
    pub fn read_node_data(&mut self, nid: NodeId) -> Result<Vec<u8>> {
        let nbt_entry = self.nbt.get(nid)
