...
//!
//! Host copy so CI `cargo test -p dedupe-chrome` executes the encode/decode gate
//! (UI crate is workspace-excluded).
...
///
/// Leptos Router `ParamsMap` already URL-decodes — treat `id_param` as the
/// absolute matter root and only encode for the href segment.
...
mod tests {
    ...
    fn stub_back_href_reencodes_decoded_windows_param() {
        // ParamsMap values are already decoded roots.
    ...
    fn literal_percent_in_root_not_double_decoded_from_params() {
        // Root contains literal "%20" and "%25" as path characters.
        let root = r"C:\Cases\100%20Done\%25done";
        let encoded_for_url = encode_matter_id(root);
        // Simulate ParamsMap: one URL unescape of the path segment.
        let params_value = decode_matter_id(&encoded_for_url).expect("router unescape");
        assert_eq!(
            params_value, root,
            "after one decode, param must equal the original root"
        );
        // Building href from ParamsMap must encode the decoded root — no second decode.
        let href = matter_home_href_from_param(&params_value);
        let enc = href.trim_start_matches("/matters/");
        assert_eq!(enc, encoded_for_url);
        assert_eq!(decode_matter_id(enc).expect("decode"), root);
        // Double-decode would turn "%20" into a space and break the path.
        let wrongly_double = decode_matter_id(&params_value).expect("lenient decode");
        assert_ne!(
            wrongly_double, root,
            "documenting the bug: decoding a ParamsMap value again mutates literal %"
        );
    }
}
