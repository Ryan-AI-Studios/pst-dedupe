...
/// Remove one root from the MRU. Does not stat the filesystem.
pub fn recent_matters_forget_in(dir: &Path, root: &str) -> Result<Vec<RecentMatter>, CommandError> {
    let mut matters = recent_matters_list_in(dir)?;
    matters.retain(|m| m.root != root);
    persist_matters(dir, matters)
}
...
mod tests {
    ...
    fn mru_front_cap_20_and_inject_dir() {
        ...
        // 0148: list_in does not stat. Prune is forget-after-not_found, not a load-time drop.
        assert!(!Path::new("root-24").exists());
        assert!(
            list.iter().any(|m| m.root == "root-24"),
            "list_in must keep unsatted synthetic roots"
        );
    ...
    #[test]
    fn forget_in_removes_root_without_bom() {
        let tmp = tempdir().expect("tempdir");
        let dir = tmp.path();
        recent_matters_remember_in(dir, r"C:\gone", "Gone").expect("rem");
        recent_matters_remember_in(dir, r"C:\keep", "Keep").expect("rem2");
        let list = recent_matters_forget_in(dir, r"C:\gone").expect("forget");
        assert_eq!(list.len(), 1);
        assert_eq!(list[0].root, r"C:\keep");
        assert!(!list.iter().any(|m| m.root == r"C:\gone"));
        let bytes = fs::read(dir.join(FILE_NAME)).expect("read");
        assert!(
            !bytes.starts_with(&[0xEF, 0xBB, 0xBF]),
            "forget write must not start with UTF-8 BOM"
        );
        let listed = recent_matters_list_in(dir).expect("list");
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].root, r"C:\keep");
    }

    #[test]
    fn forget_permission_and_capability() {
        let toml = include_str!("../permissions/autogenerated/recent_matters_forget.toml");
        assert!(toml.contains("allow-recent-matters-forget"));
    ...
    #[test]
    fn forget_missing_root_is_idempotent() {
        let tmp = tempdir().expect("tempdir");
        let dir = tmp.path();
        recent_matters_remember_in(dir, "keep", "Keep").expect("rem");
        let list = recent_matters_forget_in(dir, "absent").expect("forget");
        assert_eq!(list.len(), 1);
        assert_eq!(list[0].root, "keep");
    }
