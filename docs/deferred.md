...
| D-0080-scanpst-arg | P2 | Exact `-no repair` token + `-silent` behaviour confirmed against real Outlook builds | Microsoft's page prints the argument as `-no repair` (with a space) in both the enabling list and the arguments table. **The failure mode is asymmetric:** if none of the new arguments are recognised, scanpst falls back to its **legacy repairing** path — a typo'd flag silently repairs the file under validation. 0080 §3.6 rule 2 therefore **skips rather than guesses** until an operator run shows the log honouring it. Secondary sources also claim Outlook 2019/2021 reverted to GUI-only; unverified. Stub-executable tests cover timeout/bak/skip paths; real Outlook still operator-local | operator residual |
