...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0030-01 | — | Image/PDF box markups & burn-in redaction | **Geometric boxes + PDF burn closed in 0114** (PR **#119** / `5ed53bf`). Text path closed in 0032. **TIFF G4 closed in 0115** (PR **#121** / `19d0c1f`). | — |
...
| D-0031-09 | — | Court e-file / load-file Bates on privilege log | Chrome 0113 volume log uses Bates when `production_items` exist (PR **#117** / `f192b2d`); Desk/CLI still item_id | residual (Desk/CLI) |
...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0032-01 | — | Full PDF/image geometric redaction + content burn-in | **Closed in 0114** (PR **#119** / `5ed53bf`) for PDF content-stream burn + jpeg/png paint-burn. **TIFF G4 closed in 0115** (PR **#121** / `19d0c1f`). | — |
...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0040-01 | — | TIFF/PDF image productions + OPT/LFP | **Closed in 0115** (PR **#121** / `19d0c1f`) for TIFF G4 + Opticon OPT. LFP residual **D-0115-lfp**; PDF-as-image declined. Default DAT-only unchanged. | — |
...
| D-0060-04 | — | Image + OPT/LFP production profiles | **Closed in 0115** (PR **#121** / `19d0c1f`) — builtin `us_concordance_image_opt_v1`. LFP residual **D-0115-lfp**. | — |
...
| D-0109-also-eml-classify | P2 | unique-pst `--also-eml` combined classify: partial-allowed → `ok`/complete; summary rewrite drops also-eml cancel 130; cancel recovery zeros also-eml attach/embedded counts | **Closed in 0109:** combined fidelity worse-of (not from exit); `ok` from fidelity; summary rewrite uses `process_cancelled` → 130; cancel Ok recovers attach/embedded from summary JSON. | **closed / 0109** |
...
| D-0118-review-window-async | — | PR #115 Bugbot: stale document/body fetch overwrites current item; post-save codes/notes snapshot; unused path_id `#[test]` | **Closed in 0118** (PR **#127** / `74fd797`). Owner HITL: release EXE, synthetic 3-doc family, rapid next + Enter-on-last un-privilege. | **closed / 0118** |
| D-0032-01 | — | Geometric PDF redaction + burn-in | **Closed in 0114** (PR **#119** / `5ed53bf`) for PDF + jpeg/png. **TIFF G4 closed in 0115** (PR **#121** / `19d0c1f`). | — |
...
| D-0040-01 | — | TIFF/OPT image factory | **Closed in 0115** (PR **#121** / `19d0c1f`) — TIFF G4 + Opticon OPT; page-level Bates; default DAT-only unchanged. LFP residual **D-0115-lfp**. | — |
| D-0060-04 | — | Image + OPT/LFP production profiles | **Closed in 0115** (PR **#121** / `19d0c1f`) — `us_concordance_image_opt_v1`. LFP residual **D-0115-lfp**. | — |
...
| D-0122-process-fold-residuals | — | PR #123 Bugbot: extract-all Busy wipes remaining PST queue; live running jobs painted as orphans at mount | **Closed in 0122** (PR **#137** / `f1810fe`) — guard before queue writes; Busy keep-queue + `busy_retry_pending`; live row Pause from current progress. Owner HITL remaining (release EXE, ≥2 PSTs). | **closed / 0122** |
...
| D-0127-embedded-depth-operator-hint | — | Default `--max-embedded-depth 3` still exit-64 / `ATTACH_DEPTH_LIMIT` on INC*-class nests | **Closed in 0127** (PR **#147** / `9082761`) — stderr names flag + configured cap; default stays 3; `D-0067` remains. | **closed / 0127** |
| D-0128-export-risk-advisory-copy | — | Complete unique still reads as `re_export_recommended` | **Closed in 0128** (PR **#147** / `9082761`) — `operator_note` after also-eml combine; 0.02 frozen; no BODY_UNAVAILABLE discount. | **closed / 0128** |
| D-0129-also-eml-phase-timings | — | `phase_timings` has no `also_eml_ms`; HITL ~393 s `unaccounted_ms` | **Closed in 0129** (PR **#147** / `9082761`) — additive `also_eml_ms`; unaccounted never fudged to 0. | **closed / 0129** |
| D-0130-qc-sample-cost | — | QC sample ~407 s (~39% of INC* wall) | **Closed in 0130** (PR **#147** / `9082761`) — stderr `qc_ms=` on ok and hard-fail; default stays `sample`. Speedup declined. | **closed / 0130** |
| D-0131-recoverable-items-operator-hint | — | 826/4055 winners from Recoverable Items / Purges | **Closed in 0131** (PR **#147** / `9082761`) — runbook opt-in `--prefer-folder-class`; owner HITL with flag skipped. | **closed / 0131** |
...
| D-0127-eml-err-stderr-assert | P3 | unique-eml late report Err emits depth hint; tests do not assert the stderr string on that path | Hint is emitted before manifest/summary fail (Codex P2 fix). String assert left. | residual polish |
| D-0127-also-eml-dual-hint | P3 | `--also-eml` may print unique-pst depth hint and a `unique-eml:`-prefixed line | Allowed dual channel (unique-pst `emit_log` + pack helper). | residual polish |
