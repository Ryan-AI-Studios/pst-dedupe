...
| D-0118-review-window-async | P3 | PR #115 Bugbot: stale document/body fetch overwrites current item; post-save codes/notes snapshot; unused path_id `#[test]` | Valid; review_window.rs / path_id.rs. Fold-in 2026-08-31: refresh when persist does not navigate (`then_next && next_id is None`). | **0118** In Progress |
...
| D-0119-produce-checklist-residuals | P3 | PR #117 Bugbot: Finalize re-arm / colliding Bates; empty `filter_ids` dumps privilege log; QC state survives matter change. Plus PR #123: cancelled produce treated as success | Valid; produce.rs + privilege.rs. Do not steal into 0114/0115/0117. | **0119** Proposed |
