...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0078-retryable | P3 | Transient (retry-safe) vs permanent failure — as a **`retryable: bool` JSON field, not a new exit code** | Code residual remains (no `retryable` field yet). **0081 runbook constraint satisfied:** forbids blanket retry of exit 5; PowerShell switch example diagnoses `AuditChainBroken` / schema / passphrase vs transient IO. | residual code; **runbook constraint / 0081** |
