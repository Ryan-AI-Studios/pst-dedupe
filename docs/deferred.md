...
| D-0093-recipient-tc-multipage | P3 | Recipient TC beyond single-page HN (Strategy B budget cap) | **Closed in 0100** (PR #90, `ab1c7b0`): Strategy A row-matrix subnode + RowsPerBlock (`Floor(8176/56)=146`) + multi-block HN on the recipient-table node; shared `TableContext::load`. Production does not emit `RECIPIENT_TC_TRUNCATED`. Residuals: attach-table TC, HNBITMAPHDR, SLBLOCK NID order (**0103**). | **closed / 0100** |
...
| D-0100-slblock-nid-order | P3 | Recipient-table SLBLOCK unsorted when cell NIDs exist | PR #90 Bugbot: `table_subs.insert(0, matrix_nid)` after monotonic cell NIDs. `add_subnode_leaf` does not sort. Outlook searches SLBLOCK by NID — long-string cells can miss. Short-name INC* 136-row class often never allocates cell NIDs. **Owner: 0103 (Proposed).** Not 0101. | residual / **0103** |
