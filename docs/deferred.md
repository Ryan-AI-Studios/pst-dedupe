...
| D-0093-recipient-tc-multipage | P3 | Recipient TC beyond single-page HN (Strategy B budget cap) | **0093 locked Strategy B** (budget-aware cap + honesty). **0100 In progress 2026-08-27:** Strategy A = row-matrix subnode + RowsPerBlock (§2.3.4.4) + multi-block HN on the recipient-table node (HNPAGEHDR; HID `hidIndex` 11-bit **per page**). Shared `TableContext::load` (four concat sites). Message-PC 3580 restore stays residual (not 0100). INC* post-0098: 39 msgs / 3082 truncated rows. **Close when 0100 DoD lands.** | **0100** (In progress) |
...
| D-0094-inc-resmoke | P3 | Operator re-smoke INC0102784 unique-pst after nested export | **2026-08-26 post-0098:** 4055/4055 verify; folder_tree_match true; exit 64 `ATTACH_SOFT_FAIL` only (4 depth-limit). `export_risk` poly-class lie **closed in 0099**. Recipient truncation 39/3082 → **0100** (Ready). Depth CLI → **0101**. Optional post-0100 INC* : `output/inc0102784-post-0100/` (operator-local). | residual / **0100–0101** |
...
| D-0100-hn-bitmap-hdr | P3 | HNBITMAPHDR pages (8, 136, 264, …) not implemented in 0100 multi-block HN | 0100 fail-closes if a recipient TC heap would land on a bitmap page. INC* 136-row class is expected &lt; 8 pages. Implement only if a corpus hits the error. | residual / after **0100** |
