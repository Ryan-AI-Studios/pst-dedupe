...
From `C:\dev\deviations.md` vs `C:\dev\dedupe-frontend` (research only). **0123 Completed**. **0124 In progress**. Expand **0125–0126** with `/plan-track`. Do not steal 0119–0122 Bugbot. **0123 locks (2026-09-01):** Plex + ink-navy `#1b3049` (blue theme); Home under the shared bar. Do not port Archivo/coral.
...
| D-0124-review-queue-chrome | P3 | Review left rail + toolbar + bulk + row range. **Column text collides** (no ellipsis; X500 From overflows Fam/Resp; 32px rows must not wrap) | Live `app.css` `.queue-row` tracks; viewport `overflow-x: hidden`. Keep 0111 Control# / PRIV locks. Absorb in **0124**: `minmax(0,…)` + ellipsis + `title`; 244px rail; `#queue-goto`; SQL-page `Rows a–b of N`. | **0124** In progress |
