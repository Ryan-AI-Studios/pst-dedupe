...
| D-0093-recipient-tc-multipage | P3 | Recipient TC beyond single-page HN (Strategy B budget cap) | **Closed in 0100** (PR #90, `ab1c7b0`): Strategy A row-matrix subnode + RowsPerBlock (`Floor(8176/56)=146`) + multi-block HN on the recipient-table node; shared `TableContext::load`. Production does not emit `RECIPIENT_TC_TRUNCATED`. Residuals: attach-table TC, HNBITMAPHDR. | **closed / 0100** |
...
| D-0094-inc-resmoke | P3 | Operator re-smoke INC0102784 unique-pst after nested export | **2026-08-26 post-0098:** 4055/4055 verify; folder_tree_match true; exit 64 `ATTACH_SOFT_FAIL` only (4 depth-limit). `export_risk` poly-class lie **closed in 0099**. Recipient truncation **closed in 0100**. Depth CLI → **0101**. Optional post-0100 INC*: `output/inc0102784-post-0100/` (operator-local). | residual / **0101** |
