...
| D-0067-embedded-depth | — | Full recursive nested MAPI extract for embedded messages | P0 labels `message/rfc822` + streams raw bytes; `embedded_message_unparsed` honesty flag; max depth 3 residual. **Narrowed by 0080** (not closed): QC samples embedded-attachment messages and cross-checks `embedded_message_unparsed` / `embedded_depth_limit_hits` against what the output actually contains, so the honesty flags stop being self-reported | residual / measured **0080** |
...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0068-01 | P3 | No subnode-diversion path for oversized non-body string props (subject/sender/display_to/message_class) | Only `body_plain`/`body_html` divert to a subnode above `MAX_HEAP_VALUE_SIZE`; an oversized non-body string hard-fails the whole batch write via `?` (fail-safe, not silent truncation) rather than diverting or skipping just that message. **Narrowed by 0080** (not closed): the risk-weighted sample always includes the longest-subject and longest-sender message, so this hard-fail class is exercised on every QC run instead of remaining theoretical | future polish / exercised **0080** |
...
| D-0068-04 | — | Recipient table, attach table schema, full named-prop set | **Attach table closed in 0069** (NBT template 0x671 + per-message TC + RowIndex). Residual: recipient table + full named-prop set beyond store stub. | residual (recipient/named-prop) |
...
| D-0070-multi-source-stream-prefix | P3 | Multi-source folder prefixes on streaming path use sources **seen so far** | When `multi_source_prefix` and ≥2 distinct sources, prefixes match `unique_source_prefixes` for the current source set. Messages written **before** a second source appears may lack a source prefix (collect-all fidelity differs). Flat policy unaffected. Collect-based `plan_folder_tree` remains for unit comparison. **Narrowed by 0080**: QC samples one message per distinct source PST and compares the **folder tree** (not just summed counts), making a missing prefix an observed finding rather than a documented theory | residual polish / observable **0080** |
...
| D-0080-recipient-table | P2 | Real recipient table (SMTP + `PidTagRecipientType`) in written PSTs | 0080 §3.11 ships `PidTagDisplayCc` as a stopgap only. Joins **D-0068-04** (recipient/named-prop residual) and **D-0076-recipient-table** (which needs it for X.500/display-name identity variance) — one writer track should close all three | residual / writer |
| D-0080-bcc-policy | — | Whether unique-pst should ever write `PidTagDisplayBcc` | **Product/disclosure decision, not a writer bug.** `CanonicalMessage.display_bcc` exists (`keepset.rs:695`) and is dropped. Writing BCC into a deliverable can disclose recipients that the custodian's own copy would reveal; 0047 already defaults to visible-only (to+cc). 0080 declares it `dropped_by_design` and **counts** it; an opt-in flag needs a product call | product |
| D-0080-scanpst-arg | P2 | Exact `-no repair` token + `-silent` behaviour confirmed against real Outlook builds | Microsoft's page prints `-no repair` (with a space). **Asymmetric failure:** unrecognized args fall into the **legacy repairing** path. 0080 **skips rather than guesses**. CI: Ok only when build ≥ 16.0.10325.20082 from a real version source (`.version` / `PST_DEDUP_SCANPST_BUILD`) **and** `-no repair` is proven via help text **or** stub log line `NO_REPAIR_MODE` + success markers — never bare `.accepts-no-repair` / env. Real Outlook still operator-local. | operator residual |
...
| D-0080-newoutlook | — | No mount/automation successor once classic Outlook retires | New Outlook offers PST **import, not mount**, and Microsoft has stated it has **no plans to continue developing .PST support** once bulk import ships. PST remains a correct *deliverable* format (Purview emits it, eDiscovery consumes it) — but the *proof* strategy cannot lean on Microsoft's client, which is why 0080 makes source-differential reader QC the durable signal. Revisit when bulk import ships | product / watch |
| D-0080-cloud-attachments | P2 | Cloud/modern (OneDrive/SharePoint link) attachment payload completeness | **Q10:** `pst-reader` has no named-property resolution; cannot detect `PidNameAttachmentProviderType`. Documented as explicit `dropped_by_design` blind spot in `fidelity_contract_v1` — never silently `preserved`. Real fix needs named-prop resolution (own track) | residual / reader |
| D-0080-unexplained-byte-edit | P3 | Byte-edit PST yielding `unexplained_loss` (vs `defect` on preserved fields) | **By design:** `unexplained_loss` is allowlist fail-closed for property names absent from `fidelity_contract_v1`. Production path: `record_classified_finding` + digest `extra_source_props` (QC product sidecar, not a probe). Truncated/corrupt PST ⇒ `defect`. Inventing an uncontracted MAPI prop via pure byte-edit is not a realistic writer failure mode. | residual / design |
