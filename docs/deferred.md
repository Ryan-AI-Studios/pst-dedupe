...
| D-0093-recipient-tc-multipage | P3 | Recipient TC beyond single-page HN (Strategy B budget cap) | **Closed in 0100** (PR #90, `ab1c7b0`): Strategy A row-matrix subnode + RowsPerBlock (`Floor(8176/56)=146`) + multi-block HN on the recipient-table node; shared `TableContext::load`. Production does not emit `RECIPIENT_TC_TRUNCATED`. Residuals: attach-table TC, HNBITMAPHDR (SLBLOCK NID order closed in **0103**). | **closed / 0100** |
