...
| ID | Severity | Item | Notes | Owner |
|---|---|---|---|---|
| D-0068-01 | P3 | No subnode-diversion path for oversized non-body string props (subject/sender/display_to/message_class) | Only `body_plain`/`body_html` divert to a subnode above `MAX_HEAP_VALUE_SIZE`; an oversized non-body string hard-fails the whole batch write via `?` (fail-safe, not silent truncation) rather than diverting or skipping just that message | future polish |
| D-0068-02 | — | `scanpst.exe` / Outlook-open structural verification (§3.9 DoD-11) | No Windows Outlook/scanpst available in this build environment; EntryID (`PidTagIpmSubtreeEntryId`) and page `wSig` are real MS-PST-shaped implementations but not independently verified against real Outlook/Inbox Repair Tool | operator — required before any "Outlook production-ready" claim; run scanpst on a multi-XBLOCK fixture PST and record result in `review.md` |
| D-0068-03 | — | CLI `pst-dedup write-pst` subcommand | Spec marks CLI as preferred-not-required (§3.11); DoD gate is library API + tests, which is met. Full CLI wiring is **0071** (`unique-pst` glue: keep-set → write-pst → report) | **0071** |
| D-0068-04 | — | Recipient table, attach table schema, full named-prop set | **Narrowed** (round 9): this row previously also bundled "full MS-PST named-property template objects" and folder `PidTagContentUnreadCount`/`PidTagSubfolders` under one orchestrator "not required" judgment call — both are **now implemented** (see D-0068-05/06 below and `docs/pst-writer-fidelity-v1.md`), on newly-verified authoritative MS-PST source data that supersedes that judgment call. What remains genuinely deferred is **spec-explicit**: recipient table, attach table, and "full named-prop set" are named verbatim in spec.md §3.3's "Out v1" line ("recipient table; attach table; full named-prop set"), with attach schema additionally assigned to track 0069 by §4. | 0069 / future richness track |
