...
- **Body-only / inline paste cloud links (0085/0088/0097):** document-shaped SharePoint/OneDrive URLs in HTML/plain body (no MAPI attachment-table row) are **detected offline** and ledged in **`export_body_cloud_links.csv`** (reason `BODY_CLOUD_LINK`) with full query string preserved for as-sent native re-collection. Count on `export_messages.body_cloud_link_count`. **Not** classified as `ATTACH_CLOUD_LINK` and **not** `is_attach_incomplete` — Mode A will **not** promote solely for body hits. **Known gap:** physical attach on a peer is preferred over attachment-table CloudLink incompleteness, but physical attach is **not** preferred over HTML-inline-only via Mode A. Document-shaped filter only (not bare intranet site roots; folder `:f:` excluded). **Host allowlist:** commercial (`*.sharepoint.com`, `*.sharepoint-df.com`, `onedrive.live.com`, `1drv.ms`) **plus US GCC High / DoD** (`*.sharepoint.us` including `-my.`, `admin.onedrive.us`, `*.sharepoint-mil.us`, `*.dps.mil`). `admin.onedrive.us` is a harmless sync/admin include — operators should **not** expect body-cloud rows from it. **GCC Moderate** uses commercial endpoints (no extra hosts). **21Vianet** (`*.sharepoint.cn`) and future **`.microsoft` TLD** content hosts are out of scope (**D-0088-usgovcloud-microsoft-tld**). SafeLinks unwrap covers commercial `*.safelinks.protection.outlook.com` and GCC High/DoD `*.safelinks.protection.office365.us` when the nested target is document-shaped; current Microsoft behavior no longer wraps SharePoint/OneDrive URLs (Learn Safe Links about, 2026-05-22) — unwrap is mainly **historical mail**. **D-0085-sovereign-cloud-hosts** closed in 0088. Caps (defaults): 100k body window / 2048 URL / 50 links — **do not treat a large empty-URL CSV as 62 lost SharePoint links.** **0097 honesty:** `truncated` is a row-type discriminator. Real rows keep full query. Window-only bodies with no document-shaped candidate emit **0** CSV rows (`body_scan_window_capped_messages` still increments). When a candidate was actually dropped, **≤1** marker per message (`link_index=4294967295`) with reason `BODY_CLOUD_LINK_WINDOW` / `BODY_CLOUD_LINK_MAX_LINKS_EXCEEDED` / `BODY_CLOUD_LINK_URL_TRUNCATED` (pipe-join). The umbrella `BODY_CLOUD_LINK_TRUNCATED` string is gone. A 2048-char prefix on a URL-over-length marker is **not** a live URL. `body_cloud_link_truncated_messages` counts dropped-candidate messages only. No hydration / no invented attach objects.
- Mode A does **not** change grouping (who is a peer) — only which peer wins export after grouping. With 0084, Mode A can prefer a peer with **physical** attach bytes over a link-only (attachment-table) winner in the same keep-set group.
- **Attach-content identity (`--strong-content-hash body-recip-attach`, 0086):** Enable when attach-**byte** fidelity must drive grouping (same name:size, different file bytes). Full-stream SHA-256 per attach under dedicated budgets (`--strong-hash-attach-max-*`; default 50k attaches / 1 GiB / 512 MiB per attach). Cloud-link / unread attaches use **Choice B** domain-separated name+size sentinels (not empty digests, not tier-downgrade). This level can place complete physical and incomplete/cloud copies into **different keep-set groups** — Mode A cannot cross that split and is **not** a substitute for attach-content identity. Default identity remains `off` (no new attach I/O). Combining with `--identity-ignore-inline-attachments` softens the byte-strict promise (stderr warning; logos/signatures still filterable).
...
| Artifact | Role |
|---|---|
| `summary.json` | `unique_export_report_v1` — fidelity, exit, `export_risk`, `phase_timings`, digests, `retryable`, `bcc_suppressed_message_count`, body-cloud counters (**0085/0097**: `messages_with_body_cloud_links`, `body_cloud_links_total`, `body_cloud_link_truncated_messages`, `body_scan_window_capped_messages`), `store_record_key_mode` (**0087**, default `deterministic`) |
...
| `export_body_cloud_links.csv` | Body-inline document-shaped cloud URL hit-list (**0085/0097**; always when report pack written; `truncated` discriminator; honesty markers use `link_index=u32::MAX`) |
