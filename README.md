...
| `matter-core` | Matter layout + SQLite (schema v7: Normalized Item + dedupe/thread/neardup/cull/promote result cols + `cull_presets` + `review_sets`) + CAS (`put_bytes` / streaming `put_reader`) + audit + jobs + logical_hash v1 + `workspace/temp/` |
