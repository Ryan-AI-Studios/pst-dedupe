...
- **Plan-of-record reference:** `C:\dev\Dedupe-plan.md` → Series A / track 0017
...
Canonical reviewable item schema + parent/attachment family graph, including **physical (`native_sha256`) vs logical (`logical_hash`) identity** per `Dedupe-plan.md` §2.3.
...
1. Normalized Item Rust types + SQLite mapping (0015).
2. Family graph: parent email ↔ attachment children.
3. **Logical hash** computation for email and non-email (normalized body/participants/time; **not** raw MIME/PST bags).
4. **Native hash** of stored blob bytes for custody.
5. Unit tests: same logical content → same `logical_hash`; different transport wrappers must not be required for equality when logical inputs match.
6. Document field list aligns with `Dedupe-plan.md` §2.
