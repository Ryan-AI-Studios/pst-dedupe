...
- [ ] Re-read PR #121 Bugbot (OPT resume / all volumes / JPEG path / TIFF MIME). Confirm last PRs (window now includes **#133**) still have no product findings.
...
- [ ] Set selection per spec §2.9 (explicit id **only if it intersects** the selection; else in-progress intersecting; else exactly-one complete image set; else skip leftover completes). Skip disk TIFF checks when `output_root` is missing.
- [ ] Chrome: `intended_qc_params` / `produce_qc_run_blocking` pass set id only when the set **intersects `ordered`** (prefer non-complete intersecting, else latest complete intersecting with existing root, else omit). Do not touch `volume_succeeded`. Host test: a stale `partial` that does not overlap `ordered` is not passed.
...
- [ ] `matter-qc` depends on `pdf-raster`; `is_qc_image_eligible` calls `is_image_eligible_native` after native-only kind check. Delete path-first JPEG/PNG branch. **Delete** unused `looks_like_{pdf,jpeg,png,tiff}_magic` helpers (`rules.rs` ~500–517) so clippy `-D warnings` stays green.
