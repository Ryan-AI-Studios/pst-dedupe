...
- **Plan-of-record reference:** `C:\dev\Dedupe-plan.md` → Series A / track 0019
...
Cancelable **and resumable** background jobs with durable checkpoints, progress events, and single-process ownership by the Desk app (no user daemons).
...
1. Job state machine: Pending/Running/Paused/Failed/Cancelled/Succeeded.
2. **Checkpoint API** (stage + cursor + completed counts) durable in SQLite.
3. Cancel tokens + progress channels to UI/CLI.
4. **CPU/blocking pool** boundary for heavy work (plan §4.6).
5. Resume path: skip completed items / continue from cursor after crash simulation.
6. Tests: cancel mid-job; kill-and-resume fixture.
