# 0029 — Keyword FTS search

- **Track ID:** 0029-KeywordFtsSearch
- **Execution repo:** `C:\dev\dedupe`
- **Governance:** this directory in `C:\dev\dedupe\conductor\`
- **Plan-of-record reference:** `C:\dev\Dedupe-plan.md` → Series C / track 0029
...
Embedded **Tantivy** full-text search over item text (SQLite remains metadata only — no FTS5-primary path).
...
1. Per-matter Tantivy index under `Matters/<id>/index/`.
2. Index body/subject/title/attachment names; query API for review list.
3. Incremental index updates as process pipeline completes items (blocking pool commits).
4. Explicit non-goal: SQLite FTS5 as primary full-text engine (plan §4.7).
5. Rebuild-index maintenance command for corruption recovery.
6. Tests with multi-item corpus + simple Boolean/keyword queries.
...
- **P1 (blocking):** Dependencies: **0017,0019**
...
| Risk | Mitigation |
|---|---|
| Scope creep into full Nuix parity | Stick to Series C outcome; defer via Proposed tracks |
...
- [ ] **DoD-1 — Capability:** The Series C outcome for this track is implemented and exercisable on Windows without manual server setup.
