...
Ranked for affidavit-grade unique-PST: CRC class first, then full recipient TCs, then nested-depth flag. **No BCC track** (0082 default suppress stays). Hermes Series O frontend, if started, uses **0105+**. **0101 Completed** (PR **#92** `4bbf620`). **0102 Completed** (oracle `export_risk.inputs` attest; PR pending merge). **0103** is a thin SLBLOCK-order placeholder (PR #90 Bugbot).
