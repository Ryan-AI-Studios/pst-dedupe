...
Ranked for affidavit-grade unique-PST: CRC class first, then full recipient TCs, then nested-depth flag. **No BCC track** (0082 default suppress stays). Hermes Series O frontend, if started, uses **0105+**. **0101 Completed** (PR **#92** `4bbf620`). **0102 In progress** (oracle `export_risk.inputs` attest; PR #89 Bugbot). **0103** is a thin SLBLOCK-order placeholder (PR #90 Bugbot).
...
| [0102-ExportOracleInputsAttest](0102-ExportOracleInputsAttest/spec.md) | **In progress** | Recursive oracle `"inputs"` strip deletes `export_risk.inputs` so 0099 attest pointers never compare. Remove `"inputs"` from `SUMMARY_ALLOWLIST_KEYS`; blank job-level `/inputs` at root only. Closes **D-0099-oracle-inputs-attest**. |
| [0103-RecipientTcSlblockNidOrder](0103-RecipientTcSlblockNidOrder/spec.md) | **Proposed** | 0100 `table_subs.insert(0, matrix_nid)` leaves SLBLOCK unsorted once cell NIDs exist. Closes **D-0100-slblock-nid-order**. After 0101 unless a corpus misses long recipient strings. |

**Suggested order:** **0099** → **0100** → **0101** → **0102** → **0103**.
