...
Minted **2026-09-24** from a read-only operator smoke of Desktop `INC0102784.pst` + `INC0102784-2.pst` (4.00 GiB + 537 MiB Permute; 4099 msgs / 4055 unique). Metadata scan ~3 s; unique-pst wall remains ~13–17 min historically. **0139 Completed** (PR **#154** / `234f4fa`). **0140** Ready after 2026-09-25 `/plan-track`. **0141–0147** still placeholders until `/plan-track`. Nested `Desktop\Desktop` path **declined** (operator FS; `\\?\` works). No BCC-default. Schema **41**.
...
| [0140-ScanStderrCadence](0140-ScanStderrCadence/spec.md) | **In progress** | Scan/dups/keep-set stderr: periodic `-v` aggregates, not per-folder; `--crc-log-limit 0` silences probe writeln. Owns stderr I/O tax. Closes **D-0140-scan-stderr-cadence**. |
| [0141-KeepSetJsonEnvelope](0141-KeepSetJsonEnvelope/spec.md) | **Proposed — placeholder** | `keep-set --json` stats envelope; winners in `--keep-set-json`. Closes **D-0141-keepset-json-envelope**. |
...
**Suggested order:** **0140 Ready**. **0141–0144** operator copy/JSON (cheap, still Proposed) → **0145** reuse → **0147** attach coverage → **0146** write hotspots (HITL). Next free ID **0148** (Series Y).
