...
**Timing (2026-09-01):** unique-export Series S is closed. **0110 Completed** (PR **#111** / `5a76f0b`). **0111 Completed** (PR **#113** / `3c4ca65`). **0112 Completed** (PR **#115** / `81a3aad`). **0113 Completed** (PR **#117** / `f192b2d`). **0114 Completed** (PR **#119** / `5ed53bf`). **0115 Completed** (PR **#121** / `19d0c1f`). **0116 Completed** (PR **#123** / `727c857`). **0117 Completed** (PR **#125** / `199975c`). **0118 Completed** (PR **#127** / `74fd797`). **0119 Completed** (PR **#129** / `6a775b5`). **0120 Completed** (PR **#131** / `e87f4c1`). **0121 Completed** (PR **#135** / `600d6b3`). **0122 Completed** (PR **#137** / `f1810fe`). **0123 Completed** (PR **#139** / `fce416e`). **0124 Completed** (PR **#141** / `ff8b0ea`). **0125 Completed** (un-wizard produce canvas; 0119 latch frozen). **0126 Completed**. Do not remint IDs. Do not vendor `C:\dev\dedupe-frontend`.

**Stack lock:** Tauri 2 + Leptos, Plex/paper/cool chrome, Process chrome **0116 Completed** (Desk Process not deleted). One pipeline. No daemon. Image/OPT **0115 Completed** (opt-in profile; default DAT-only stays). Search builder folds into **0111** (no extra search-builder ID). **0117** is PR #113 queue Bugbot, not search. **0118** is PR #115 window async, not produce. **0119** is PR #117 produce wizard Bugbot + PR #123 cancelled-as-success, not raster. **0120** is PR #119 Image-tab/Burn-count Bugbot, not OPT. **0121** is PR #121 image QC/eligibility Bugbot, not Process. **0122** is PR #123 Process extract-all/orphan Bugbot, not queue. **0123–0126** are mockup layout (Series T), not Bugbot. No BCC-default track.

| Track | Status | Summary |
|---|---|---|
| [0110-MatterChromeTauri](0110-MatterChromeTauri/spec.md) | **Completed** | Tauri 2 + Leptos matter list/home; one `matter_overview` (`load_case_overview`). Plex/paper. Desk Process stays. Closes **D-0110-matter-chrome**. Residual **D-0110-deny-unic**. PR **#111** `5a76f0b`. |
...
| [0112-ReviewWindow](0112-ReviewWindow/spec.md) | **Completed** | Three-pane coding; Responsiveness ⊥ Privilege; Native/Text; Image stub. Closes **D-0112-review-window**. PR **#115** `81a3aad`. |
| [0113-ProduceChecklist](0113-ProduceChecklist/spec.md) | **Completed** | Chrome produce wizard + QC checklist; DAT natives/text only; privilege-in-set hard block. Closes **D-0113-produce-checklist**. No OPT. PR **#117** `f192b2d`. |
| [0114-PdfRasterRedact](0114-PdfRasterRedact/spec.md) | **Completed** | `zpdf` CPU raster + geometric draft boxes + content-stream burn then rewrite. Schema **v40**. Closes **D-0032-01** / **D-0034-02**; **D-0034-04** as duplicate. Residuals **D-0114-pdfium-sidecar** / **D-0114-xform-text**. PR **#119** `5ed53bf`. |
| [0115-ImageOptFactory](0115-ImageOptFactory/spec.md) | **Completed** | TIFF G4 + Opticon OPT; page-level Bates; builtin `us_concordance_image_opt_v1`. Default DAT-only unchanged. Schema **v41**. Closes **D-0040-01** / **D-0060-04** (LFP residual **D-0115-lfp**). Residuals **D-0115-color** / **D-0115-email-print**. PR **#121** `19d0c1f`. |
| [0116-ProcessFold](0116-ProcessFold/spec.md) | **Completed** | Swallow egui Process into the same Tauri window (`process-runner`); absorb D-0113-long-job. Closes **D-0116-process-fold** / **D-0113-long-job**. PR **#123** `727c857`. |
| [0117-QueueVirtualizationResiduals](0117-QueueVirtualizationResiduals/spec.md) | **Completed** | PR **#113** Bugbot: queue header/spacer, empty-page vacant lie, arrow `scroll_top`. Fold-in 2026-08-31. PR **#125** `199975c`. |
| [0118-ReviewWindowAsyncResiduals](0118-ReviewWindowAsyncResiduals/spec.md) | **Completed** | PR **#115** Bugbot: stale document/body fetch; post-save snapshot; unused path_id `#[test]`. Fold-in 2026-08-31 (did-not-navigate refresh; catalog gated). PR **#127** `74fd797`. |
| [0119-ProduceChecklistResiduals](0119-ProduceChecklistResiduals/spec.md) | **Completed** | PR **#117** Bugbot: Finalize re-arm, empty privilege-log `filter_ids`, QC state across matters. Plus PR **#123** cancelled/idle treated as success. `volume_succeeded` latch; empty-union blank 0. Not 0125 canvas. PR **#129** `6a775b5`. |
| [0120-PdfRasterUiResiduals](0120-PdfRasterUiResiduals/spec.md) | **Completed** | PR **#119** Bugbot: overlay mouseup frame coords, draw cancel, Burn-set recount. Not OPT. Not 0119 latch. PR **#131** `e87f4c1`. |
| [0121-ImageOptQcResiduals](0121-ImageOptQcResiduals/spec.md) | **Completed** | PR **#121** Bugbot: OPT skip until complete, scoped image QC, JPEG eligibility, TIFF magic vs MIME. Not 0116. Not 0120 overlay. PR **#135** `600d6b3`. |
| [0122-ProcessFoldResiduals](0122-ProcessFoldResiduals/spec.md) | **Completed** | PR **#123** Bugbot: extract-all Busy must not wipe queue; live job-row Pause from current `progress`. Not 0117. Not 0119 cancelled-produce. Not 0126 table. PR **#137** `f1810fe`. |

**Suggested order:** **0110** → **0111** → **0112** → **0113**; **0114** after window; **0115 Completed**; **0116 Completed** (Process last on the spine). **0117 Completed** (queue Bugbot). **0118 Completed** (window async). **0119 Completed** (produce honesty). **0120 Completed** (raster UI honesty). **0121 Completed** (image QC honesty). **0122 Completed** (Process extract-all / orphan). **0123 Completed** (shell). **0124 Completed** (queue chrome). **0125 Completed** (produce canvas). **0126 Completed** (Process visual).

## Series T — Mockup chrome fidelity (post-0122)

Bugbot residuals **0120–0122** stay their own tracks. These IDs steal **layout** from `C:\dev\dedupe-frontend` (research only). **0123 Completed**. **0124 Completed**. **0125 Completed**. **0126 Completed**. **0123 tokens locked 2026-09-01:** Plex + ink-navy `#1b3049` (blue theme); Home under the shared bar. Do not port Archivo/coral.

| Track | Status | Summary |
|---|---|---|
| [0123-MatterShell](0123-MatterShell/spec.md) | **Completed** | Shared TopBar + StatusBar; Home under the bar; Plex + ink-navy `#1b3049`. Admin inert span; recents BOM. PR **#139** / `fce416e`. Not 0124 columns. |
| [0124-ReviewQueueChrome](0124-ReviewQueueChrome/spec.md) | **Completed** | 244px rail, toolbar, bulk, Go-to in 0123 right slot, SQL-page row range in status-left. Cell ellipsis so X500 From never paints on Fam/Resp/PRIV. Control# = `review_order`; PRIV coding. No 0117 math rewrite. PR **#141** / `ff8b0ea`. |
...
## Series U — Unique-export INC* HITL residuals (post-0126)

Operator unique-pst smoke **2026-09-02** on Desktop `INC0102784.pst` + `INC0102784-2.pst` (one search split at 4GB) after Series T, release CLI at `f8cb240`: exit **0**, fidelity `complete`, **4055/4055**, `folder_tree_match` true, depth **8**, also-eml 4055/446/0. Wall **1043 s**. Not a correctness regression vs 0107 HITL (787 s). Source PSTs stay out of git. **No BCC-default track.** Do not mint `D-0108-keepset-crc-retaint` or `D-0067-embedded-depth` matter children. Placeholders expanded **2026-09-03** (`/plan-track 127-132`). Shipped **2026-09-03**.

| Track | Status | Summary |
|---|---|---|
| [0127-EmbeddedDepthOperatorHint](0127-EmbeddedDepthOperatorHint/spec.md) | **Completed** | Depth-limit stderr names `--max-embedded-depth=` + cap; default stays **3**. `D-0067` remains. PR **#147** / `9082761`. |
...
**Suggested order:** **0133 Completed**. **0134 Completed**. **0135 Completed**. **0136 Completed**. **0137 Completed**.

## Series W — Chrome IPC argument case (post-V)

Working tree on `73f1d7f` (2026-09-24): uncommitted `tauri_invoke` camelCase rewrite. Tauri 2.11.5 default looks up camelCase command args; UI serde fields are snake_case; host has no `rename_all`. One-word args still work. Underscored args (`params_json`, `job_id`, `filter_json`, …) are dropped. **Shallow only** — nested `WarningOverride` / `ChromeQcFinding` stay snake_case. No BCC-default. Schema **41**.
...
Minted **2026-09-24** from a read-only operator smoke of Desktop `INC0102784.pst` + `INC0102784-2.pst` (4.00 GiB + 537 MiB Permute; 4099 msgs / 4055 unique). Metadata scan ~3 s; unique-pst wall remains ~13–17 min historically. **0139 Completed** (PR **#154** / `234f4fa`). **0140 Completed** (PR **#156** / `0839c38`). **0141 Completed** (PR **#158** / `766f523`). **0142 Completed** (PR **#160** / `a4dae1e`). **0143 Completed** (PR **#162** / `1617720`). **0144–0147** still placeholders until `/plan-track`. Nested `Desktop\Desktop` path **declined** (operator FS; `\\?\` works). No BCC-default. Schema **41**.
...
| [0143-PolyCrcScanCopy](0143-PolyCrcScanCopy/spec.md) | **Completed** | Additive `poly_crc_note` + stderr `note:` when poly-class CRC; preflight math unchanged. Closes **D-0143-poly-crc-scan-copy**. PR **#162** / `1617720`. |
...
**Suggested order:** **0143 Completed**. **0144** integrity CSV (still Proposed) → **0145** reuse → **0147** attach coverage → **0146** write hotspots (HITL). Next free ID **0148** (Series Y).
...
| Track | Status | Summary |
|---|---|---|
| [0148-RecentsOpenHonesty](0148-RecentsOpenHonesty/spec.md) | **Proposed — placeholder** | Missing roots / folders without `matter.db` fail closed; do not `remember`. Closes **D-0148-recents-open-honesty**. |
| [0149-PickerDialogReset](0149-PickerDialogReset/spec.md) | **Proposed — placeholder** | New matter / Open / Add PST dialogs do not inherit leftover search. Closes **D-0149-picker-dialog-reset**. |
| [0150-ProcessPauseOnce](0150-ProcessPauseOnce/spec.md) | **Proposed — placeholder** | One Pause per live busy job (row vs footer duplicate). Closes **D-0150-process-pause-once**. |
| [0151-ProcessIngestProgress](0151-ProcessIngestProgress/spec.md) | **Proposed — placeholder** | Honest ingest percent/bar; 0134 extract bar stays. Closes **D-0151-process-ingest-progress**. |
| [0152-ProduceShellHonesty](0152-ProduceShellHonesty/spec.md) | **Proposed — placeholder** | Locked 3-pane overflow; canvas on error; Finalize not `.primary` when disabled. Closes **D-0152-produce-shell-honesty**. |
| [0153-ReviewQueryAndFacets](0153-ReviewQueryAndFacets/spec.md) | **Proposed — placeholder** | Query chip + Reading as + inert-honest facets. Closes **D-0153-review-query-facets**. |
| [0154-ReviewBulkCheck](0154-ReviewBulkCheck/spec.md) | **Proposed — placeholder** | Select all matching (or inert) + CHECK from privilege preview. Closes **D-0154-review-bulk-check**. |
| [0155-ReviewFamilyRows](0155-ReviewFamilyRows/spec.md) | **Proposed — placeholder** | Attachment indent/marker; no fake QC/redaction columns. Closes **D-0155-review-family-rows**. |
| [0156-ReviewNamedQueues](0156-ReviewNamedQueues/spec.md) | **Proposed — placeholder** | Later rail: real filters or omit fake zeros. Closes **D-0156-review-named-queues**. |
| [0157-ReviewGotoBates](0157-ReviewGotoBates/spec.md) | **Proposed — placeholder** | Go-to Bates when an index exists; else keep Control#/subject. Closes **D-0157-review-goto-bates**. |
