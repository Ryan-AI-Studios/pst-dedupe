...
Ranked for affidavit-grade unique-PST: CRC class first, then full recipient TCs, then nested-depth flag. **No BCC track** (0082 default suppress stays). Hermes Series O frontend, if started, uses **0105+**. **0101 spec expanded 2026-08-27** (Ready). **0102** is a thin oracle-attest placeholder (PR #89 Bugbot). **0103** is a thin SLBLOCK-order placeholder (PR #90 Bugbot; do not steal into 0101).
...
| [0101-EmbeddedDepthFlag](0101-EmbeddedDepthFlag/spec.md) | **In Progress** | Wire `--max-embedded-depth` (clap reject outside 1–8, default 3) on unique-pst. Same value to materialize + writer. Fold-in 2026-08-27: DoD-2 is 4@3 vs 4@4 and 8@7 vs 8@8 (no CLI chain-of-9). Narrows unique-pst half of **D-0067-embedded-depth**. Identity hash depth stays 3. |
