...
| [0079-MaterializeWritePerformance](0079-MaterializeWritePerformance/spec.md) | **In Progress** | Measurement-gated perf: `PhaseTimings` in summary + **export equivalence oracle** (output is *not* byte-reproducible — store record key uses clock+pid); single-materialize (winners are materialized **twice** today); O(1) AMap bookkeeping (per-block rescan is O(blocks×pages) — quadratic at multi-GB); positioned block writes (no `BufWriter` over a seeking writer); one bounded LRU handle cache (closes D-0074-mat-lru); `--jobs` opt-in **only if needed** — dedicated threads not rayon, **source-affinity partitioning mandatory** (items are already source-clustered; work-stealing would make `--jobs 4` slower), `crc_attribution: aggregate` for D-0077-parallel-attrib. mmap declined for sources *and* output temp |
