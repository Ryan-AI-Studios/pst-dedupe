...
- **Plan-of-record reference:** `C:\dev\Dedupe-plan.md` → Series A / track 0016
...
Detect/import Purview export folders and multi-ZIP packages with path-traversal and zip-bomb guards, plus **entry-level expand checkpoints** for large packages.
...
1. Package detector (Purview-like layout vs raw dump vs single PST).
2. ZIP/7z recursion with path safety (reject `..`, absolute paths, symlinks).
3. Zip-bomb limits (ratio, size, entry count) and streaming extract where feasible.
4. Checkpoint after each fully expanded top-level entry (resume-friendly).
5. **Fuzz or property tests** for path normalization / zip safety (plan §5.5).
6. Audit events for import start/end and failures.
