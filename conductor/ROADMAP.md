...
Post-0098 INC* soak: verify counts match; handoff still not affidavit-clean. **0101 spec expanded 2026-08-27** (Ready). Expand 0102/0103 before coding those. **Do not** add a BCC-write track (0082 default suppress is the disclosure policy).
...
| **0101** | EmbeddedDepthFlag | **Ready** | **P1 P** | CLI `--max-embedded-depth` (reject outside 1–8, default 3). Same value to materialize + writer. CI proves 4@3 vs 4@4 and 8@7 vs 8@8 (writer cannot emit a 9th nest). Narrows unique-pst **D-0067-embedded-depth**. Identity hash depth stays 3. Spec: `0101-EmbeddedDepthFlag/`. |
